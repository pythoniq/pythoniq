...
def serialize(v):
    ...
    if v is None:
        ...
        return 'a:%d:{%s};' % (len(v), ''.join(
            serialize(k) + serialize(v) for (k, v) in (isinstance(v, dict) and v.items() or enumerate(iter(v)))))
    # TODO: Objects?
    else:
...
#     ]
#     tests += [
#         (float('0.0'), 'Zero decimal'),
#         (float('10.01'), 'Positive decimal'),
#         (float('-10.01'), 'Negative decimal'),
#     ]
#     tests += [
